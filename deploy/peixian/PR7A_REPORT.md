...
## 后续获准本机集成验收（2026-09-17）

用户授权“下面进行验收，验收通过后进行 PR-b”后，使用全新 `synthetic-pr7a-local`，2 名额、5 合成普通账号，独立端口 19447/14098 完成真实容器验收。原有部署未升级。以下结果补充并更新下文初检阶段的 not_run 状态，不改变原 PR6 生产阻断。

- 注册 5 账号，零运行资源及名额；显式启动两个账号，6 个真实 Agent/Gateway/Relay 容器；其余三个满额拒绝并保持元数据。重复启动没有新建第二任务。
- A 停止后仅 B 的三容器运行；A 的停止期间 Skill 修改没有唤醒环境；重新启动加载新版本且会话及合成文件正文保留；旧状态停止被拒绝。
- 浏览器 1366/1920/390 宽度未横向溢出，未启动状态及中文草稿可见，未自动唤醒或发送。截图在本机忽略目录 `output/playwright/pr7a-*.png`。
- 冻结合成部署并停组件后，完整备份 7 卷及 Worker/配置/密钥，工具完成清单和校验值核对。仅清理本次已备份的测试资源后恢复到 `synthetic-pr7a-restored` 空目标；保留原合成控制卷及完整备份。恢复撤销旧认证、保持冻结；重新登录确认 5 身份、零占用，再显式启用调度及启动，原会话 ID 和文件正文匹配，三个未落地账号保持元数据。
- 实测发现并修复两个工具缺口：新卷漏部署／账号标签；恢复 Compose 的 PX_CONTROL_URL 未更新。后者初次恢复容器健康但 gate=open_pending，修复后用同一重写函数修正隔离恢复目录、正常 API 停启复验通过。未伪称首次恢复即成功。新建真实三卷验证新标签；旧卷按登记 Runtime ID 兼容清单采集。
- 工具回归：228 passed、2 skipped、163 subtests passed；专项 6 passed。Control/前端代码未变，沿用下文源码回归。容器使用 b538aceab 构建；本轮修正仅宿主工具，无容器源码变动。最终工具版本以本验收提交为准。

脱敏结果见 `reports/pr7a-local-acceptance.json`；初检脚本为 `pr7-local-acceptance.py`。本机 PR-7A 可进入 PR-7B，不能据此声称 Linux、压力、生产迁移或付费模型验收通过。验收结束已停止本次恢复实例及 Worker，持久数据与私密备份保留。未推送远端。
