...
空数据库可直接初始化。现有按需库切换必须先冻结、解决所有任务与恢复责任、备份并停止旧组件，再在单次获准启动中设置 `PX_ALLOW_POOL_POLICY_CHANGE=1`；这不是在线管理 API。启动后移除该临时许可。禁止通过改配置为 false 静默关闭已启用策略；不能降版本或用旧镜像直连。既有非按需部署不改变行为。
