#!/usr/bin/env python3
"""Run the bounded two-user, seven-tool Agent acceptance exactly once.

The script creates one new acceptance session per ordinary test account.  It
does not alter any existing session, skill, file, model configuration, or
plugin configuration.  It stores only session IDs and safe pass/fail evidence
in the deployment private state so a failed model request is never retried
automatically.
"""
...
STATE_KEY = "synthetic_records_tool_acceptance"
...
def evidence(answers, expected_module: str, fixture_profile="legacy"):
...
def main():
    ...
    try:
        if STATE_KEY in api.state:
            raise RuntimeError("seven-tool agent acceptance was already started; do not retry automatically")
        model_id = api.state.get("model")
        if not isinstance(model_id, str) or not model_id:
            raise RuntimeError("an approved test model is required")
        api.state[STATE_KEY] = {"sessions": {}, "cases": {}}
        api.save()
        clients = {}
        for account, _, _ in CASES:
            if account not in clients:
                clients[account] = api.login(account)
                session = api.request(clients[account], "POST", "/sessions", json={"title": "七项合成资料插件验收"})
        ...
        for account, module, label in CASES:
            ...
            api.request(clients[account], "POST", f"/sessions/{session_id}/messages", json={"text": prompt, "model_id": model_id})
            try:
                result = evidence(wait_for_answer(api, clients[account], session_id), module, arguments.fixture_profile)
