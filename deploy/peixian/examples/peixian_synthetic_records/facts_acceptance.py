"""Publish code-prepared skills; total budget 40 includes four prior requests.
...
REPORT = DEPLOY / "facts-acceptance-20260918.json"
...
def package():
    ...
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as z:
        for name in ("manifest.json", "entry.mjs", "engine.mjs", "legacy.mjs", "fixtures.json"):
            ...
            z.writestr(info, (ROOT / "facts-plugin" / name).read_bytes())
...
def main():
    ...
    parser.add_argument("--batch", choices=["v201"], default="v201")
    args = parser.parse_args()
    report = json.loads(REPORT.read_text()) if REPORT.exists() else {"model_limit": 40, "prior_requests": 4, "cases": [], "prepared": {}, "scope": "code_prepared_scenarios"}
    ...
    try:
        ...
        if args.action == "prepare":
            ...
            if not any(x["id"] == PID and x["version"] == "1.2.0" for x in published):
                api.request(su, "POST", "/admin/plugins", files={"file": ("scenario-1.2.0.zip", blob, "application/zip")})
            conns = api.request(su, "GET", "/admin/connections")["items"]
            connection = next(x for x in conns if x["name"] == "沛县七项合成资料服务")
            api.request(su, "PUT", "/admin/plugins/"+PID+"/1.2.0/connections", json={"bindings": {"peixian_records": connection["id"]}})
            ...
            api.request(client, "PUT", "/plugins/"+PID, json={"version": "1.2.0", "enabled": True, "config": {}})
