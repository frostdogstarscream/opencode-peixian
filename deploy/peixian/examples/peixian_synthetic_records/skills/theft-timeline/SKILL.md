---
name: 盗窃案件时空资料核对（合成演示）
description: 整理固定合成场景的已知记录、来源和无法核对项目，不判定个人违法犯罪。
---

版本：1.0.0。仅支持 DEMO-CASE-THEFT，所有内容是合成演示资料。
先调用 peixian_get_scenario_context，参数 {"scenario_id":"DEMO-CASE-THEFT"}。
依次调用以下资料工具，每个只调用一次、参数为空：
- peixian_get_night_records
- peixian_get_portrait_records
- peixian_get_vehicle_records
