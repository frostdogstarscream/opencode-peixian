const METHOD="GET", PATH="/system/multiDimension/idCard/";
export default async function plugin(_context,_options,platform) {
 return {tool:{peixian_query_warning_detail:{description:"读取已确认范围的来源预警详情。条件由平台冻结，无需参数。",args:{},async execute(args){
  const request=args?.request;
  if(!request || request.method!==METHOD || !["DEMO-PERSON-001","DEMO-PERSON-002"].some(id=>request.path===PATH+id))throw new Error('查询范围未冻结或接口不匹配。');
