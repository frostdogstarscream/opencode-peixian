"""PR-5.6 immutable source, registry and evidence closeout checks."""
import argparse
import hashlib
import json
...
FILES=['stage2-pr56-'+name for name in ['contract.md','acceptance.md','registry.json','release.json']]
PATHS=['services/peixian-control','deploy/peixian/platform-facts','deploy/peixian/stage2-pr56-release-check.py','deploy/peixian/tests/test_stage2_pr56_release.py','.github/workflows/peixian-stage2-capability-rules.yml']
BASELINE='8d33ec3d5ae61143164803ce3406e9b72a1819ba'
WORKFLOW='Peixian PR-5.6 Capability Rule checks'
...
def validate(root,candidate=False):
    ...
    release=json.loads((specs/'stage2-pr56-release.json').read_text())
    require(release['baseline']==BASELINE and release['stage']=='pr56' and release['schema_version']==6,'release_identity')
    ...
    lines=(specs/'stage2-pr56-SHA256SUMS.txt').read_text().splitlines()
    ...
    registry=json.loads((specs/'stage2-pr56-registry.json').read_text())
    for name in ('capabilities','rules','methods'):
        require(registry[name]==json.loads((root/'services/peixian-control/control/developer_registry/data'/f'{name}.json').read_text()),'registry_drift')
    historical=['specs/stage2-pr5-*','specs/stage2-pr55-*','deploy/peixian/stage2-pr5-release-check.py','deploy/peixian/stage2-pr55-release-check.py']
...
if __name__=='__main__':
    ...
    validate(Path(__file__).resolve().parents[2],a.candidate);print('PR-5.6 candidate valid' if a.candidate else 'PR-5.6 closeout valid')
