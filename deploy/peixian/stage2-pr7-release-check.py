"""PR-7 immutable source, registry and evidence closeout checks."""
import argparse
import hashlib
import json
...
FILES=['stage2-pr7-'+name for name in ['contract.md','acceptance.md','clarification-cases.jsonl','openapi.json','release.json']]
PATHS=['services/peixian-control','deploy/peixian/platform-facts','deploy/peixian/platform-config.py','deploy/peixian/Backend.Control.Dockerfile','deploy/peixian/stage2-pr7-release-check.py','deploy/peixian/tests/test_stage2_pr7_release.py','.github/workflows/peixian-stage2-clarification.yml']
BASELINE='cb8e9cdcd9af997fe0c75d1283f178cf3c410b3e'
WORKFLOW='Peixian PR-7 Clarification checks'
...
def validate(root,candidate=False):
    ...
    release=json.loads((specs/'stage2-pr7-release.json').read_text())
    require(release['baseline']==BASELINE and release['stage']=='pr7' and release['schema_version']==8,'release_identity')
    ...
    lines=(specs/'stage2-pr7-SHA256SUMS.txt').read_text().splitlines()
    ...
    cases=[json.loads(line) for line in (specs/'stage2-pr7-clarification-cases.jsonl').read_text().splitlines()]
    require(len(cases)==8 and len({c['id'] for c in cases})==8,'context_corpus')
    ...
    historical=['specs/stage2-pr5-*','specs/stage2-pr55-*','specs/stage2-pr56-*','specs/stage2-pr6-*','deploy/peixian/stage2-pr6-release-check.py','deploy/peixian/stage2-pr5-release-check.py','deploy/peixian/stage2-pr55-release-check.py']
...
if __name__=='__main__':
    ...
    validate(Path(__file__).resolve().parents[2],a.candidate);print('PR-7 candidate valid' if a.candidate else 'PR-7 closeout valid')
