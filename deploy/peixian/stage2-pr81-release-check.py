"""Check PR-8.1 immutable code, migration ancestry and exact CI receipts."""
import argparse,hashlib,json,re,subprocess
from pathlib import Path
BASELINE='f0e8009b9f909a3798b7c1fb83ee9282402f714f'
FILES=['stage2-pr81-'+x for x in ['contract.md','acceptance.md','openapi.json','release.json']]
PATHS=['packages/peixian-console','services/peixian-control','deploy/peixian/platform-config.py','deploy/peixian/Backend.Control.Dockerfile','deploy/peixian/stage2-pr81-release-check.py','deploy/peixian/tests/test_stage2_pr81_release.py','.github/workflows/peixian-stage2-result-integration-review.yml']
WORKFLOW='Peixian PR-8.1 Result Integration Review checks'
...
def artifacts(specs,candidate):
    release=json.loads((specs/'stage2-pr81-release.json').read_text())
    require(release['stage']=='pr81' and release['baseline']==BASELINE and release['schema_version']==9,'identity')
    ...
    entries={}
    for line in (specs/'stage2-pr81-SHA256SUMS.txt').read_text().splitlines():
    ...
    doc=json.loads((specs/'stage2-pr81-openapi.json').read_text())
...
def validate(root,candidate=False):
    ...
    old+=['specs/stage2-pr5*','specs/stage2-pr6*','specs/stage2-pr7*','specs/stage2-pr8a*','specs/stage2-pr8b*']
...
if __name__=='__main__':
    ...
    candidate=args.candidate or (args.auto and json.loads((root/'specs/stage2-pr81-release.json').read_text())['closeout_status']=='pending')
    validate(root,candidate);print('PR-8.1 candidate validated' if candidate else 'PR-8.1 closeout validated')
