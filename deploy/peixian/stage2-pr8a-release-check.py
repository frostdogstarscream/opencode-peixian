"""Check PR-8A immutable code, migration ancestry and exact CI receipts."""
import argparse,hashlib,json,re,subprocess
from pathlib import Path
BASELINE='77d3f05647abee50fc1185a2441c1ae4f686a7d1'
FILES=['stage2-pr8a-'+x for x in ['contract.md','acceptance.md','openapi.json','release.json']]
PATHS=['services/peixian-control','deploy/peixian/platform-config.py','deploy/peixian/Backend.Control.Dockerfile','deploy/peixian/stage2-pr8a-release-check.py','deploy/peixian/tests/test_stage2_pr8a_release.py','.github/workflows/peixian-stage2-trusted-result.yml']
WORKFLOW='Peixian PR-8A Trusted Result checks'
...
def artifacts(specs,candidate):
    release=json.loads((specs/'stage2-pr8a-release.json').read_text())
    require(release['stage']=='pr8a' and release['baseline']==BASELINE and release['schema_version']==9,'identity')
    ...
    entries={}
    for line in (specs/'stage2-pr8a-SHA256SUMS.txt').read_text().splitlines():
    ...
    doc=json.loads((specs/'stage2-pr8a-openapi.json').read_text())
...
def validate(root,candidate=False):
    ...
    old=['services/peixian-control/control/migrations_v'+str(v)+'.py' for v in range(4,9)]
    old+=['specs/stage2-pr5*','specs/stage2-pr6*','specs/stage2-pr7*']
...
if __name__=='__main__':
    ...
    candidate=args.candidate or (args.auto and json.loads((root/'specs/stage2-pr8a-release.json').read_text())['closeout_status']=='pending')
    validate(root,candidate);print('PR-8A candidate validated' if candidate else 'PR-8A closeout validated')
