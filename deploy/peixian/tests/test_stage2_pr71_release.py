...
spec=importlib.util.spec_from_file_location('pr7',ROOT/'deploy/peixian/stage2-pr71-release-check.py')
check=importlib.util.module_from_spec(spec);spec.loader.exec_module(check)
def git(root,*args):return subprocess.check_output(['git',*args],cwd=root,text=True,stderr=subprocess.PIPE).strip()
def save(root,m):
 (root/'specs/stage2-pr7-integration-release.json').write_text(json.dumps(m))
 fpath=root/'specs/stage2-pr7-integration-findings.json';f=json.loads(fpath.read_text());f['baseline']=m['baseline']
 for item in f['findings']:item['fix_commit']=m['implementation_revision']
 fpath.write_text(json.dumps(f))
 (root/'specs/stage2-pr7-integration-SHA256SUMS.txt').write_text(''.join(hashlib.sha256((root/'specs'/n).read_bytes()).hexdigest()+'  '+n+'\n' for n in check.FILES))
@pytest.fixture
def release(tmp_path,monkeypatch):
 root=tmp_path/'repo';root.mkdir()
 for name in check.FILES:
  p=root/'specs'/name;p.parent.mkdir(exist_ok=True);shutil.copyfile(ROOT/'specs'/name,p)
 p=root/'services/peixian-control/control';p.mkdir(parents=True);(p/'store.py').write_text('MAX_SCHEMA_VERSION = 8');(p/'migrations_v8.py').write_text('fixture')
 ...
 m={'stage':'pr71','schema_version':8,'closeout_status':'completed','baseline':baseline,'implementation_revision':revision,'deployment_performed':False,'real_data_accessed':False,'model_requests':0,'github_ci':{'executed':True,'workflow':check.WORKFLOW,'head_sha':revision,'run_id':123,'conclusion':'success'},'tests':{k:{'exit_code':0} for k in ('control_gateway','deployment','bun','historical')}}
...
def test_open_finding_rejected(release):
 root,m=release;p=root/'specs/stage2-pr7-integration-findings.json';f=json.loads(p.read_text());f['findings'][0]['status']='open';p.write_text(json.dumps(f));save(root,m)
 with pytest.raises(ValueError,match='open_findings'):check.validate(root)


def test_v8_image_preserves_runtime_pool_protocol():
 spec=importlib.util.spec_from_file_location('platform_config_pr7',ROOT/'deploy/peixian/platform-config.py')
 config=importlib.util.module_from_spec(spec);__import__('sys').modules[spec.name]=config;spec.loader.exec_module(config)
 labels={'org.peixian.runtime.protocol':'2','org.peixian.worker.protocol':'2','org.peixian.control.schema.max':'8','org.peixian.worker.capabilities':config.pool_settings.CAPABILITY}
