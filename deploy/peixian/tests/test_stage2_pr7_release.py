...
spec=importlib.util.spec_from_file_location('pr7',ROOT/'deploy/peixian/stage2-pr7-release-check.py')
check=importlib.util.module_from_spec(spec);spec.loader.exec_module(check)
def git(root,*args):return subprocess.check_output(['git',*args],cwd=root,text=True,stderr=subprocess.PIPE).strip()
def save(root,m):
 (root/'specs/stage2-pr7-release.json').write_text(json.dumps(m))
 (root/'specs/stage2-pr7-SHA256SUMS.txt').write_text(''.join(hashlib.sha256((root/'specs'/n).read_bytes()).hexdigest()+'  '+n+'\n' for n in check.FILES))
@pytest.fixture
...
def test_corpus_drift(release):
 root,m=release;p=root/'specs/stage2-pr7-clarification-cases.jsonl';p.write_text('');save(root,m)
