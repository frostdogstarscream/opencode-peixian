...
export function ProviderQuery(props:{model:string;disabled:boolean;sessionID?:string;runID?:string;onAccepted:(sid:string)=>void}) {
 ...
 return <Show when={choices().some(c=>c.available)}>
   ...
   <Show when={result()?.versions?.query}><details class="provider-records"><summary>查看本轮来源与下一步查询（{result()?.records?.length??0} 条）</summary><p>仅依据选定记录继续；先核对时间和范围，再确认发起。资料不用于人员嫌疑评分。</p>
     <For each={result()?.records}>{row=><article><strong>{row.record_id}</strong><dl><For each={Object.entries(row.fields)}>{([k,v])=><><dt>{({cjbh:"处警编号",cjsj:"处警时间",address:"来源地址",target_name:"来源对象",target_id_card:"对象引用",capture_count:"抓拍汇总次数",deviceName:"来源地点",captureTime:"观测时间",idCard:"对象引用",personName:"来源名称",warningTypes:"来源预警类型",warningCount:"来源类型数",deviceId:"设备引用",trackType:"来源轨迹类型",latestTime:"最近触发时间",warningType:"来源预警类型",count:"来源规则触发次数",gisX:"来源经度字段",gisY:"来源纬度字段",longitude:"来源经度字段",latitude:"来源纬度字段",logs:"来源明细",communityName:"来源区域名称",id:"来源编号"} as Record<string,string>)[k]??k}</dt><dd>{typeof v==="object"?JSON.stringify(v):String(v??"未提供")}</dd></>}</For></dl>
...
export function OwnerReviews(props:{sessionID:string;runID:string}) {
 ...
 let generation=0;onCleanup(()=>generation++)
 createEffect(()=>{const sid=props.sessionID,rid=props.runID,p=page();tick();const g=++generation;setData(undefined);setError("");setSupersedes("")
 ...
 <Field label={supersedes()?"更正意见（保留原记录）":"复核意见"}><textarea maxlength={2000} value={note()} onInput={e=>setNote(e.currentTarget.value)} /></Field><ErrorLine message={error()}/><Button disabled={busy()||locked()||!note().trim()} onClick={()=>void save()}>追加复核记录</Button><Button onClick={()=>{setTick(x=>x+1);setLocked(false)}}>刷新复核状态</Button><Button disabled={page()<=1} onClick={()=>setPage(x=>x-1)}>上一页</Button><span>第 {page()} 页</span><Button disabled={page()*20>=(data()?.total??0)} onClick={()=>setPage(x=>x+1)}>下一页</Button>
 </details></Show>
}
