...
export default function TheftTaskQuery(props:{model:string;disabled:boolean;sessionID?:string;runID?:string;onAccepted:(sid:string)=>void}) {
 ...
 createEffect(()=>{const sid=props.sessionID,rid=props.runID,g=++epoch;setRecords([]);setSelected([]);setDigest("");setScope({});setGoal("");setError("");setUncertain(false)
  if(sid&&rid)void Promise.all([api<{records:RecordRow[]}>(`/sessions/${sid}/runs/${rid}/result`),api<{result_digest:string}>(`/sessions/${sid}/runs/${rid}/reviews`)]).then(([r,d])=>{if(g===epoch){setRecords(r.records??[]);setDigest(d.result_digest)}}).catch(()=>{})
 })
