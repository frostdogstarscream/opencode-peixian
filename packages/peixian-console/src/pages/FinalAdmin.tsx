...
export default function FinalAdmin(props: { section: Section }) {
  ...
  const [page, setPage] = createSignal(1), [pageSize, setPageSize] = createSignal(10)
  const [selectedFilters, setSelectedFilters] = createSignal<Record<string, string>>({})
  let refreshGeneration = 0
  let auditModelsRequested = false
  let modelFormElement!: HTMLFormElement

  const shownModels = createMemo(() => filter(models(), query(), (item) => `${item.name} ${item.model_id} ${item.provider} ${item.description}`).filter((item) =>
    (!selectedFilters().provider || item.provider === selectedFilters().provider) &&
    (!selectedFilters().status || String(!!item.enabled) === selectedFilters().status)))
  const shownDepartments = () => departments()
  const shownUsers = createMemo(() => filter(users(), query(), (item) => `${item.display_name} ${item.police_no} ${item.department?.name} ${item.username}`).filter((item) =>
    (!selectedFilters().department || item.department_id === selectedFilters().department) &&
    (!selectedFilters().role || item.role === selectedFilters().role) &&
    (!selectedFilters().status || String(!!item.active) === selectedFilters().status)))
  const userSummary = () => summary()
  const paging = () => ({ page: page(), pageSize: pageSize(), setPage, setPageSize })
  function setSearch(value: string) { setPage(1); setQuery(value) }
  function setFilter(key: string, value: string) { setPage(1); setSelectedFilters((current) => ({ ...current, [key]: value })) }
  function resetFilters() { setPage(1); setQuery(""); setSelectedFilters({}) }
  function auditParameters(paginated = true) {
    const params = new URLSearchParams()
    if (paginated) { params.set("page", String(page())); params.set("page_size", String(pageSize())) }
    if (query().trim()) params.set("query", query().trim())
    const filters = selectedFilters()
    if (filters.model_id) params.set("model_id", filters.model_id)
    if (filters.status) params.set("status", filters.status)
    if (filters.start) params.set("start", new Date(`${filters.start}T00:00:00`).toISOString())
    if (filters.end) params.set("end", new Date(new Date(`${filters.end}T00:00:00`).getTime() + 86400000).toISOString())
  ...
  async function refresh() {
    ...
    try {
      if (props.section === "models") {
        const values = await list<Model>("/admin/models")
        if (generation === refreshGeneration) setModels(values)
      }
      ...
      if (props.section === "audit") {
        if (!auditModelsRequested) {
          auditModelsRequested = true
          void list<Model>("/admin/models").then(setModels).catch(() => {})
        }
        const values = await api<{ items: Invocation[]; total: number }>("/admin/invocations?" + auditParameters())
        if (generation !== refreshGeneration) return
        setInvocations(values.items)
        setAuditTotal(values.total)
      }
    } catch (cause) { if (generation === refreshGeneration) setError(safeMessage((cause as Error).message)) }
  }
  createEffect(on(() => props.section, () => resetFilters()))
  createEffect(on(() => [app.changed(), props.section], () => { void refresh() }))
  createEffect(on(() => [props.section, page(), pageSize(), query(), selectedFilters()], () => {
    if (props.section !== "audit") return
  ...
  return <div class="police-admin-page">
    <Switch>
      ...
      <Match when={props.section === "audit"}><AdminHero icon="clock" title="调用审计" text="记录模型、Skill 与插件调用链路，支持问题追溯与安全审计。" action={<a class="button" href={download("/admin/invocations/export" + (auditParameters(false) ? "?" + auditParameters(false) : ""))} download="">导出记录</a>} /></Match>
    </Switch>
    <ErrorLine message={error()} />
    <Show when={props.section === "models"}><ModelsPage models={shownModels()} allModels={models()} query={query()} setQuery={setSearch} filters={selectedFilters()} setFilter={setFilter} reset={resetFilters} paging={paging()} edit={(item) => { setConnectionState("idle"); setModelForm(item) }} test={testModel} setEnabled={setModelEnabled} /></Show>
    <Show when={props.section === "users"}><UsersPage users={shownUsers()} departments={shownDepartments()} summary={userSummary()} query={query()} setQuery={setSearch} filters={selectedFilters()} setFilter={setFilter} reset={resetFilters} paging={paging()} departmentsMode={showDepartments()} setDepartmentsMode={setShowDepartments} departmentsWritable={app.user().role === "super_admin"} editUser={setUserForm} editDepartment={setDepartmentForm} deleteDepartment={deleteDepartment} resetPassword={resetUserPassword} disableUser={disableUser} /></Show>
    <Show when={props.section === "audit"}><AuditPage items={invocations()} models={models()} total={auditTotal()} query={query()} setQuery={setSearch} filters={selectedFilters()} setFilter={setFilter} reset={resetFilters} paging={paging()} show={showInvocation} /></Show>
...
type Paging = { page: number; pageSize: number; setPage: (value: number) => void; setPageSize: (value: number) => void }

function ModelsPage(props: { models: Model[]; allModels: Model[]; filters: Record<string, string>; setFilter: (key: string, value: string) => void; reset: () => void; paging: Paging; query: string; setQuery: (value: string) => void; edit: (item: Model) => void; test: (item: Model) => Promise<void> | void; setEnabled: (item: Model, enabled: boolean) => Promise<void> | void }) {
  return <section class="admin-content-card"><FilterBar query={props.query} setQuery={props.setQuery} placeholder="搜索模型名称、ID 或描述..." selects={[{ key: "provider", label: "全部 Provider", options: [...new Set(props.allModels.map((item) => item.provider).filter((value): value is string => !!value))].map((value) => ({ value, label: value })) }, { key: "status", label: "全部状态", options: [{ value: "true", label: "已启用" }, { value: "false", label: "已停用" }] }]} filters={props.filters} setFilter={props.setFilter} reset={props.reset} /><Table><thead><tr><th>模型名称</th><th>Provider</th><th>Model ID</th><th>上下文长度</th><th>接入方式</th><th>状态</th><th>默认</th><th>更新时间</th><th>操作</th></tr></thead><tbody><For each={props.models.slice((props.paging.page - 1) * props.paging.pageSize, props.paging.page * props.paging.pageSize)}>{(item) => <tr><td><div class="model-cell"><span class="model-logo">{item.name.slice(0, 1)}</span><span><strong>{item.name}</strong><small>{item.description}</small></span></div></td><td><span class="provider-tag">{item.provider || "其他"}</span></td><td>{item.model_id}</td><td>{Math.round((item.context_length ?? 131072) / 1024)}K</td><td><span class="access-tag">{item.access_mode === "local" ? "本地部署" : "API 接入"}</span></td><td><div class="status-actions"><Button class={item.enabled ? "selected" : ""} disabled={item.enabled} onClick={() => void props.setEnabled(item, true)}>启用</Button><Button class={!item.enabled ? "selected danger-selected" : ""} disabled={!item.enabled} onClick={() => void props.setEnabled(item, false)}>关停</Button></div></td><td>{item.is_default ? <span class="default-tag">默认模型</span> : "—"}</td><td>{formatDate(item.updated_at)}</td><td><div class="row-actions"><Button onClick={() => props.edit(item)}>编辑</Button><Button variant="ghost" onClick={() => void props.test(item)}>测试</Button></div></td></tr>}</For></tbody></Table><TableFooter total={props.models.length} paging={props.paging} /></section>
}
function UsersPage(props: { users: User[]; departments: Department[]; filters: Record<string, string>; setFilter: (key: string, value: string) => void; reset: () => void; paging: Paging; summary: { users: number; departments: number; enabled: number }; query: string; setQuery: (value: string) => void; departmentsMode: boolean; setDepartmentsMode: (value: boolean) => void; departmentsWritable: boolean; editUser: (item: Partial<User>) => void; editDepartment: (item: Partial<Department>) => void; deleteDepartment: (item: Department) => Promise<void> | void; resetPassword: (item: User) => Promise<void> | void; disableUser: (item: User) => Promise<void> | void }) {
  return <><div class="admin-summary-cards"><Summary icon="users" label="用户总数" value={props.summary.users} note="当前接口实时数据" /><Summary icon="skill" label="部门总数" value={props.summary.departments} note="当前接口实时数据" /><Summary icon="shield" label="启用账号" value={props.summary.enabled} note="当前接口实时数据" /></div><section class="admin-content-card"><div class="subsection-tabs"><button class={!props.departmentsMode ? "active" : ""} onClick={() => props.setDepartmentsMode(false)}>用户管理</button><button class={props.departmentsMode ? "active" : ""} onClick={() => props.setDepartmentsMode(true)}>部门管理</button></div><div class="admin-list-toolbar"><FilterBar query={props.query} setQuery={props.setQuery} placeholder="搜索姓名、警号或部门..." selects={[{ key: "department", label: "全部部门", options: props.departments.map((item) => ({ value: item.id, label: item.name })) }, { key: "role", label: "全部角色", options: [{ value: "user", label: "普通用户" }, { value: "admin", label: "管理员" }, { value: "super_admin", label: "超级管理员" }] }, { key: "status", label: "全部状态", options: [{ value: "true", label: "已启用" }, { value: "false", label: "已停用" }] }]} filters={props.filters} setFilter={props.setFilter} reset={props.reset} compact /><Show when={!props.departmentsMode || props.departmentsWritable}><Button variant="primary" icon="plus" onClick={() => props.departmentsMode ? props.editDepartment({ sort_order: 0 }) : props.editUser({ active: true, role: "user" })}>{props.departmentsMode ? "新增部门" : "新增用户"}</Button></Show></div><Show when={!props.departmentsMode} fallback={<Show when={props.departments.length} fallback={<p class="empty-copy">暂无部门。</p>}><Table><thead><tr><th>部门名称</th><th>组织编码</th><th>上级部门</th><th>排序</th><th>操作</th></tr></thead><tbody><For each={props.departments}>{(item) => <tr><td><strong>{item.name}</strong></td><td>{item.code || "—"}</td><td>{props.departments.find((value) => value.id === item.parent_id)?.name || "—"}</td><td>{item.sort_order}</td><td><Show when={props.departmentsWritable} fallback="只读"><div class="row-actions"><Button onClick={() => props.editDepartment(item)}>编辑</Button><Button variant="danger" onClick={() => void props.deleteDepartment(item)}>删除</Button></div></Show></td></tr>}</For></tbody></Table></Show>}><Table><thead><tr><th>姓名</th><th>警号</th><th>所属部门</th><th>角色</th><th>账号状态</th><th>最近登录</th><th>操作</th></tr></thead><tbody><For each={props.users.slice((props.paging.page - 1) * props.paging.pageSize, props.paging.page * props.paging.pageSize)}>{(item) => <tr><td><div class="officer-cell"><span class="officer-avatar">警</span><strong>{item.display_name || item.username}</strong></div></td><td>{item.police_no || item.username}</td><td>{item.department?.name || "—"}</td><td><span class={item.role === "super_admin" ? "role-tag leader" : "role-tag"}>{item.position || item.role}</span></td><td><Status value={item.active ? "enabled" : "disabled"} /></td><td>{formatDate(item.last_login_at)}</td><td><div class="row-actions"><Button onClick={() => props.editUser(item)}>编辑</Button><Button variant="ghost" onClick={() => void props.resetPassword(item)}>重置密码</Button><Button variant="danger" disabled={item.active === false} onClick={() => void props.disableUser(item)}>禁用</Button></div></td></tr>}</For></tbody></Table><TableFooter total={props.users.length} paging={props.paging} /></Show></section></>
}
function AuditPage(props: { items: Invocation[]; models: Model[]; total: number; filters: Record<string, string>; setFilter: (key: string, value: string) => void; reset: () => void; paging: Paging; query: string; setQuery: (value: string) => void; show: (item: Invocation) => Promise<void> | void }) {
  return <section class="admin-content-card"><FilterBar query={props.query} setQuery={props.setQuery} placeholder="搜索用户名或脱敏摘要..." selects={[{ key: "model_id", label: "全部模型", options: props.models.map((item) => ({ value: item.id, label: item.name })) }, { key: "status", label: "全部状态", options: [{ value: "completed", label: "已完成" }, { value: "failed", label: "失败" }, { value: "cancelled", label: "已取消" }, { value: "running", label: "运行中" }] }]} filters={props.filters} setFilter={props.setFilter} reset={props.reset} dateRange /><Table><thead><tr><th>时间</th><th>用户</th><th>部门</th><th>模型</th><th>请求类型</th><th>证据数</th><th>结果状态</th><th>操作</th></tr></thead><tbody><For each={props.items}>{(item) => <tr><td>{formatDate(item.created_at ?? item.created)}</td><td>{item.display_name || item.username}</td><td>{item.department_name || "—"}</td><td>{item.model_name || "—"}</td><td><span class="audit-question" title={item.query_summary}>{item.query_summary}</span></td><td>{item.record_count}</td><td><Status value={item.status} /></td><td><Button onClick={() => void props.show(item)}>查看详情</Button></td></tr>}</For></tbody></Table><TableFooter total={props.total} paging={props.paging} /></section>
}
...
function FilterBar(props: { query: string; setQuery: (value: string) => void; placeholder: string; selects: { key: string; label: string; options: { value: string; label: string }[] }[]; filters: Record<string, string>; setFilter: (key: string, value: string) => void; reset: () => void; dateRange?: boolean; compact?: boolean }) {
  return <div class={"prototype-filter-bar " + (props.compact ? "compact" : "")}>
    <label class="search-box"><Icon name="search" size={17} /><input value={props.query} onInput={(event) => props.setQuery(event.currentTarget.value)} placeholder={props.placeholder} /></label>
    <Show when={props.dateRange}><div class="audit-date-range"><input aria-label="开始日期" type="date" value={props.filters.start || ""} onChange={(event) => props.setFilter("start", event.currentTarget.value)} /><span>~</span><input aria-label="结束日期" type="date" value={props.filters.end || ""} onChange={(event) => props.setFilter("end", event.currentTarget.value)} /></div></Show>
    <For each={props.selects}>{(select) => <select aria-label={select.label} value={props.filters[select.key] || ""} onChange={(event) => props.setFilter(select.key, event.currentTarget.value)}><option value="">{select.label}</option><For each={select.options}>{(option) => <option value={option.value}>{option.label}</option>}</For></select>}</For>
    <Button icon="refresh" onClick={props.reset}>重置</Button>
  </div>
}
...
function TableFooter(props: { total: number; paging: Paging }) {
  const pageCount = () => Math.max(1, Math.ceil(props.total / props.paging.pageSize))
  const pageNumbers = () => Array.from({ length: Math.min(5, pageCount()) }, (_, index) => Math.max(1, Math.min(props.paging.page - 2, pageCount() - 4)) + index)
  return <div class="prototype-pagination"><strong>共 {props.total} 条记录</strong><div>
    <button aria-label="上一页" disabled={props.paging.page <= 1} onClick={() => props.paging.setPage(props.paging.page - 1)}>‹</button>
    <For each={pageNumbers()}>{(number) => <button class={number === props.paging.page ? "active" : ""} aria-label={`第 ${number} 页`} onClick={() => props.paging.setPage(number)}>{number}</button>}</For>
    <button aria-label="下一页" disabled={props.paging.page >= pageCount()} onClick={() => props.paging.setPage(props.paging.page + 1)}>›</button>
    <select aria-label="每页条数" value={props.paging.pageSize} onChange={(event) => { props.paging.setPage(1); props.paging.setPageSize(Number(event.currentTarget.value)) }}><option value="10">10 条/页</option><option value="20">20 条/页</option><option value="50">50 条/页</option></select>
  </div></div>
}
