...
import type { Audit, Department, Invocation, Model, Role, User } from "../types"
...
export default function FinalAdmin(props: { section: Section }) {
  ...
  async function refresh() {
    ...
    try {
      ...
      if (props.section === "users") {
        const values = await list<User>("/admin/users")
        setUsers(values)
        setDepartments([])
        setSummary({ users: values.length, departments: 0, enabled: values.filter((item) => item.active !== false).length })
      }
      if (props.section === "audit") {
        const values = await list<Audit>("/admin/audit")
        setInvocations(values.map((item) => ({
          id: item.id,
          run_id: item.id,
          created: item.created,
          username: item.username || item.actor,
          display_name: item.username || item.actor,
          skill_ids: [],
          plugin_ids: [],
          status: item.result || item.status || "unknown",
          record_count: 0,
          query_summary: [item.action, item.target, item.detail].filter(Boolean).join(" · "),
        })))
      }
  ...
  async function saveModel(event: SubmitEvent) {
    ...
    const payload = { name: data.name, model_id: data.model_id, base_url: data.base_url, api_key: data.api_key || undefined, description: data.description, enabled: data.enabled === "on", is_default: data.is_default === "on" }
    try {
  ...
  function testDraftConnection() {
    ...
    app.notify("当前后端仅支持已保存模型的连接测试；保存前测试接口尚未提供。", "error")
  }
  async function saveUser(event: SubmitEvent) {
    ...
    const payload = userForm()?.id ? { ...grants, active: data.active === "on" } : { ...grants, username: data.username, password: data.password || undefined, ...(app.can("admins.manage") ? { role: (data.system_role || "user") as Role } : {}) }
    const identityPending = [data.display_name, data.police_no, data.department_id, data.position].some(Boolean)
    try { if (userForm()?.id) await patch("/admin/users/" + userForm()!.id, payload); else await post("/admin/users", payload); setUserForm(undefined); await refresh(); app.notify(identityPending ? "账号及授权已保存；姓名、警号、部门、职务后端尚未接入。" : "用户信息已保存。", identityPending ? "error" : "success") }
    catch (cause) { setError(safeMessage((cause as Error).message)) }
  }
  ...
  async function saveDepartment(event: SubmitEvent) {
    ...
    app.notify("当前后端尚未提供部门管理接口。", "error")
  }
  ...
  return <div class="police-admin-page">
    <Switch>
      ...
      <Match when={props.section === "audit"}><AdminHero icon="clock" title="调用审计" text="记录模型、Skill 与插件调用链路，支持问题追溯与安全审计。" action={<Button icon="download" onClick={() => app.notify("当前后端尚未提供调用审计导出接口。", "error")}>导出记录</Button>} /></Match>
    </Switch>
