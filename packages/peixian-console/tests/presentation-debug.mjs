...
for(const width of [1366,1920,390]) {
 ...
 console.log("history-labels",await page.locator(".history-list button").allTextContents());await page.getByRole("button",{name:"涉赌案件资料整理",exact:true}).first().click();await page.getByRole("region",{name:"研判结果",exact:true}).waitFor();
 ...
 if(width===390)await page.getByRole("button",{name:"显示对话记录",exact:true}).click();await page.getByRole("button",{name:"盗窃案件时空资料核对",exact:true}).first().click();await result.getByText("核对车辆与地点",{exact:true}).waitFor();
