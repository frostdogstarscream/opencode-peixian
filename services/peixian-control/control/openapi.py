...
def schemas():
    ...
    result = {
        ...
        "ScenarioEvidence": obj({
            ...
            "cards": array(obj({"id": STRING, "title": STRING, "description": STRING, "time": STRING, "source_ids": array(STRING), "fields": array(obj({"label": STRING, "value": STRING}, ("label", "value"))), "message_id": STRING, "snapshot_id": STRING})),
            "missing": array(STRING), "summary": array(obj({"label": STRING, "count": INTEGER, "dates": array(STRING), "night_count": INTEGER})),
        }, ("schema_version", "status", "notice", "scenario", "steps", "cards", "missing", "summary")),
...
WORKER_CONTRACTS = {
    ...
    ('post','/internal/worker/jobs/{jid}/cancel-idle'): ('LeaseBody',ref('WorkerReceipt')),
    ('post','/internal/worker/scheduler/tick'): (None,obj({'items':array(obj({},extra=True)),'protocol_version':INTEGER},extra=True)),
    ('post','/internal/worker/scheduler/observe'): ('IdleObservationBody',obj({'accepted':BOOL,'job_id':ID},extra=True)),
