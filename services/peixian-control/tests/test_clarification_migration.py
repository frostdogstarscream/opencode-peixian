...
from control import migrations_v8
from test_control import PASSWORD

@pytest.fixture
def original(tmp_path,monkeypatch):
 monkeypatch.setenv('PX_BACKEND_V6','1');monkeypatch.setenv('PX_TASK_CONTEXT_V1','1');monkeypatch.delenv('PX_TASK_CLARIFICATION_V1',raising=False)
...
def test_upgrade_repeat_and_empty_target_restore(original,monkeypatch,tmp_path):
 ...
 new=migrate(original,monkeypatch);assert new.schema_version()==8
 assert new.one('SELECT id,password,role,auth_version FROM users')==identity
 assert Store(*args).schema_version()==8
 ...
 monkeypatch.delenv('PX_TASK_CLARIFICATION_V1')
 restored=Store(target,*args[1:]);assert restored.schema_version()==7
...
def test_unapproved_or_busy_upgrade_refused(original,monkeypatch):
 s,args,_=original;monkeypatch.setenv('PX_TASK_CLARIFICATION_V1','1')
 with pytest.raises(ValueError,match='offline approval'):Store(*args)
 assert s.schema_version()==7
 with s.tx() as db:db.execute("UPDATE platform_state SET maintenance_mode='frozen' WHERE id=1")
 monkeypatch.setenv('PX_ALLOW_V8_MIGRATION','1')
 with s.tx() as db:
...
def test_injected_failure_rolls_back_ddl(original,monkeypatch):
 s,args,_=original
 monkeypatch.setattr(migrations_v8,'DDL',migrations_v8.DDL+['INVALID SQL'])
 with pytest.raises(sqlite3.OperationalError):migrate(original,monkeypatch)
 assert s.schema_version()==7 and not s.one("SELECT 1 FROM sqlite_master WHERE name='task_clarifications'")

@pytest.mark.parametrize('change',['table','fingerprint','script'])
def test_v8_corruption_fails_closed(original,monkeypatch,change):
 s=migrate(original,monkeypatch)
 with s.tx() as db:
  if change=='table':db.execute('DROP TABLE task_clarifications')
  else:db.execute('UPDATE schema_migrations SET '+('structure_digest' if change=='fingerprint' else 'script_digest')+"='bad' WHERE to_version=8")
